 ...
 */
export const attachVideo = (path: string) => attachArtifact(path, 'video', 'video/mp4');

/**
 * Attach a screenshot artifact to the current test.
 * @param path Absolute path to the screenshot file
 */
export const attachScreenshot = (path: string) => attachArtifact(path, 'screenshot');
 ...
 */
export const attachFile = (path: string, level?: ArtifactLevel) => attachArtifact(path, undefined, undefined, level);
